L.Control.SlideMenuFarming = L.Control.extend({
    options: {
        position: 'topleft',
    ...
    onAdd: function(map){
        ...
        this._menu = L.DomUtil.create('div', 'leafletfarming-menu', map._container);
        ...
        var closeButton = L.DomUtil.create('button', 'leafletfarming-menu-close-button', this._menu);
        ...
        if(this._isHorizontal){
            if(this._isLeftPosition){
                ...
                L.DomUtil.addClass(closeButton, 'fas fa-apple-alt fa-lg');
            }
        }
        else{
            if(this._isTopPosition){
        ...
        this._contents = L.DomUtil.create('div', 'leafletfarming-menu-contents', this._menu);
    ...
    _animate: function(menu, from, to, isOpen, ispx, unit){
        ...
        if(from != to){
            setTimeout(function(slideMenuFarming){
                var value = isOpen ? from + unit : from - unit;
                slideMenuFarming._animate(slideMenuFarming._menu, value, to, isOpen, ispx, unit);
...
L.control.slideMenuFarming = function(innerHTML, options) {
    return new L.Control.SlideMenuFarming(innerHTML, options);
};
