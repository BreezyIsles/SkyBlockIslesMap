L.Control.SlideMenuMining = L.Control.extend({
    options: {
        position: 'topleft',
    ...
    onAdd: function(map){
        ...
        if(this._isHorizontal){
            if(this._isLeftPosition){
                ...
                L.DomUtil.addClass(closeButton, 'fas fa-gem fa-lg');
            }
        }
        else{
            if(this._isTopPosition){
    ...
    _animate: function(menu, from, to, isOpen, ispx, unit){
        ...
        if(from != to){
            setTimeout(function(slideMenuMining){
                var value = isOpen ? from + unit : from - unit;
                slideMenuMining._animate(slideMenuMining._menu, value, to, isOpen, ispx, unit);
...
L.control.slideMenuMining = function(innerHTML, options) {
    return new L.Control.SlideMenuMining(innerHTML, options);
};
