L.Control.SlideMenuQuests = L.Control.extend({
    options: {
        position: 'topleft',
    ...
    onAdd: function(map){
        ...
        this._menu = L.DomUtil.create('div', 'leafletquests-menu', map._container);
        ...
        var closeButton = L.DomUtil.create('button', 'leafletquests-menu-close-button', this._menu);
        ...
        if(this._isHorizontal){
            if(this._isLeftPosition){
                ...
                L.DomUtil.addClass(closeButton, 'fas fa-exclamation fa-lg');
            }
        }
        else{
            if(this._isTopPosition){
        ...
        this._contents = L.DomUtil.create('div', 'leafletquests-menu-contents', this._menu);
    ...
    _animate: function(menu, from, to, isOpen, ispx, unit){
        ...
        if(from != to){
            setTimeout(function(slideMenuQuests){
                var value = isOpen ? from + unit : from - unit;
                slideMenuQuests._animate(slideMenuQuests._menu, value, to, isOpen, ispx, unit);
...
L.control.slideMenuQuests = function(innerHTML, options) {
    return new L.Control.SlideMenuQuests(innerHTML, options);
};
