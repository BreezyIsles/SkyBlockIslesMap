L.Control.SlideMenuShops = L.Control.extend({
    options: {
        position: 'topleft',
    ...
    onAdd: function(map){
        ...
        this._menu = L.DomUtil.create('div', 'leafletshops-menu', map._container);
        ...
        var closeButton = L.DomUtil.create('button', 'leafletshops-menu-close-button', this._menu);
        ...
        if(this._isHorizontal){
            if(this._isLeftPosition){
                ...
                L.DomUtil.addClass(closeButton, 'fas fa-dollar-sign fa-lg');
            }
        }
        else{
            if(this._isTopPosition){
        ...
        this._contents = L.DomUtil.create('div', 'leafletshops-menu-contents', this._menu);
    ...
    _animate: function(menu, from, to, isOpen, ispx, unit){
        ...
        if(from != to){
            setTimeout(function(slideMenuShops){
                var value = isOpen ? from + unit : from - unit;
                slideMenuShops._animate(slideMenuShops._menu, value, to, isOpen, ispx, unit);
...
L.control.slideMenuShops = function(innerHTML, options) {
    return new L.Control.SlideMenuShops(innerHTML, options);
};
