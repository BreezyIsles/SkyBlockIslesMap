L.Control.SlideMenuWoodcutting = L.Control.extend({
    options: {
        position: 'topleft',
    ...
    onAdd: function(map){
        ...
        link.title = 'Woodcutting';
        L.DomUtil.create('span', this.options.icon, link);

        this._menu = L.DomUtil.create('div', 'leafletmining-menu', map._container);
        ...
        var closeButton = L.DomUtil.create('button', 'leafletmining-menu-close-button', this._menu);
        ...
        if(this._isHorizontal){
            if(this._isLeftPosition){
                ...
                L.DomUtil.addClass(closeButton, 'fas fa-tree fa-lg');
            }
        }
        else{
            if(this._isTopPosition){
        ...
        this._contents = L.DomUtil.create('div', 'leafletmining-menu-contents', this._menu);
    ...
    _animate: function(menu, from, to, isOpen, ispx, unit){
        ...
        if(from != to){
            setTimeout(function(slideMenuWoodcutting){
                var value = isOpen ? from + unit : from - unit;
                slideMenuWoodcutting._animate(slideMenuWoodcutting._menu, value, to, isOpen, ispx, unit);
...
L.control.slideMenuWoodcutting = function(innerHTML, options) {
    return new L.Control.SlideMenuWoodcutting(innerHTML, options);
};
